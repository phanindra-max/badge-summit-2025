$(".swap[name='searchstring']").css("color", "#ccc");
swap_val = [];
$(".swap[name='searchstring']").each(function (i) {
  swap_val[i] = $(this).val();
  $(this)
    .focusin(function () {
...
$(document).ready(function () {
  // on touch devices the Schedule dropdown doesn't work on :hover
  if (is_touch_device()) {
    var schedViewsDisplayed = false;
    $("#sched-views > a").click(function (event) {
      event.stopPropagation();
      event.preventDefault();

      if (schedViewsDisplayed) {
        schedViewsDisplayed = false;
        $("#sched-views").addClass("hover");
      } else {
        schedViewsDisplayed = true;
        $("#sched-views").removeClass("hover");
      }
    });
    $("#sched-views .popover-content a").click(function () {
      window.location.href = $(this).attr("href");
    });
  }

  $(".event.frozen .ev-save-suspended").click(function () {
    $("#sched-overlay-generic").show();
    $("#sched-overlay-frozen").show();
    $("#sched-overlay-button").hide();
    $("#sched-overlay-frozen")
      .children(".sched-overlay-button")
      .off("click")
      .click(function () {
        $(this).off("click");
        $("#sched-overlay-generic").fadeOut(200);
        $("#sched-overlay-frozen").hide();
      });
    return false;
  });

  $(".event.pinned:not(.frozen) .ev-save-suspended").click(function () {
    $("#sched-overlay-generic").show();
    $("#sched-overlay-pinned").show();
    $("#sched-overlay-button").hide();
    $("#sched-overlay-pinned")
      .children(".sched-overlay-button")
      .off("click")
      .click(function () {
        $(this).off("click");
        $("#sched-overlay-generic").fadeOut(200);
        $("#sched-overlay-pinned").hide();
      });
    return false;
  });

  if ($("#js-settings-form").length) {
    $("#sched-username").focus(function () {
      unnotify("un-ok", "Type to change your username.");
    });
    $("#sched-username")
      .keyup(function () {
    ...
    $("#js-settings-form").submit(function () {
      ...
      if (!cv && oun != $("#sched-username").data("origvalue")) {
        cv = true;
        $(".js-confirm-settings").slideDown(200, function () {
          $("#pass-confirm").focus();
        });
        doingSubmit = false;
        return false;
      ...
      if (cv) {
        if (!unok) {
          ...
          window.location = "#un-label";
          return false;
        } else if (
          $.trim($("#pass-confirm").val()) &&
          oun != $("#sched-username").data("origvalue")
        )
          return true;
        else if (oun == $("#sched-username").data("origvalue")) return true;
        else {
          // does the user need to set up a new password?
          if ($("#pwone").length) {
            if (
              $.trim($("#pwone").val()) == "" ||
              $.trim($("#pwtwo").val()) == "" ||
              $.trim($("#pwone").val()).length < 3
            ) {
              doingSubmit = false;
...
function delayedHideTooltip() {
  if (delayedHideTooltipTimer) clearTimeout(delayedHideTooltipTimer);
...
$(function () {
  ...
  if (!is_iphone) {
    ...
    // sasa@sched.org 2012-07-09 temporarily disable for all
    // until server RAM issue is resolved

    if (showfull) {
      $.getJSON(
        "/event-get-all-tooltips?page=" + page,
...
function saveEvent(lnk, removecheck, refresh) {
  if (removecheck === undefined) removecheck = true;
  if (refresh === undefined) refresh = false;

  // no need to save event if it's under a role
  // const $parentWrapper =
  //   typeof lnk === "string"
  //     ? $(`#${lnk}`).parent(".event")
  //     : $(lnk).parent(".event");
  //
  // if (
  //   $parentWrapper.attr("class") &&
  //   $parentWrapper.attr("class").match(/\bevent-role-[^\s]+/)
  // ) {
  //   return;
  // }
  ...
  if (username != "") {
    ...
    if ($("#page-event").length) {
      if ($(".sub").length) {
        $("#sched-page-event-attendees").prepend(
          '<p class="sched-page-event-attendees-note">Attendance numbers do not account for private attendees. Get there early!</p>'
        );
      } else {
        if ($("p.sched-page-event-attendees-note").length) {
          $("p.sched-page-event-attendees-note").remove();
        }
      }
    }
    ...
    if (lnk.id) {
      if ($(lnk).hasClass("sub")) {
        ...
        //lnk.innerHTML = g_abl;
        $.get("/event-set?del=" + lnk.id, function (data) {
        ...
        //lnk.innerHTML = "LEAVE";
        //lnk.innerHTML = g_lbl;
        $.get("/event-set?add=" + lnk.id, function (data) {
          alert(data);
          if (refresh) location.reload();
        });
      }
      ...
      var id = lnk;
      var prli = $("#" + id).parent();
      if ($("#" + id + " .sched-event-seats").length != 0)
        var seats = $("#" + id + " .sched-event-seats");
      else if (
        $("#" + id)
          .next()
          .children(".sched-event-seats").length != 0
      )
        var seats = $("#" + id)
          .next()
          .children(".sched-event-seats");
      else {
        $("#" + id + " .sched-event-evpeople").before(
          "<span class='sched-event-seats'></span>"
        );
        var seats = $("#" + id + " .sched-event-seats");
      }
      var pseats = $("#" + id)
        .parent()
        .parent()
        .find(".sched-event-permalink-seats");
      var psnote = pseats.parent().parent().children(".sched-event-capnotice");
      if (prli.hasClass("sub")) {
        seats = prli.children(".name").children(".sched-event-seats");
        if (
          (seats.hasClass("sched-event-seats-full") ||
            seats.hasClass("sched-event-seats-filling")) &&
          removecheck
        ) {
          $("#sched-overlay-generic").show();
          $("#sched-overlay-removeconfirm").show();
          if (isEmbed) {
            var pageHeight = Math.max($("body").height(), $("html").height());
            var overlayPos =
              prli.offset().top +
              $("#sched-overlay-generic-inner").height() / 2;
            if (overlayPos + 250 > pageHeight)
              overlayPos = overlayPos - (overlayPos + 250 - pageHeight + 25);
            $("#sched-overlay-generic-inner").css({ top: overlayPos + "px" });
          }
          $("#sched-overlay-removeconfirm")
            .children(".sched-overlay-button-red")
            .off("click")
            .click(function () {
              saveEvent(id, false);
              $(this).off("click");
              $("#sched-overlay-generic").fadeOut(200);
              $("#sched-overlay-removeconfirm").hide();
            });
          return false;
        }
        ...
        if (hasSchedule < 1) {
          ...
          $("#sched-confirm-send").hide();
        }
        $("#sched-confirm-schedule-count").html(hasSchedule);
        $.get("/event-set?del=" + id, function (data) {
          ...
          if (data == "noremove") {
            ...
            if (hasSchedule > 1) {
              ...
              $("#sched-confirm-send").show();
            }
            $("#sched-confirm-schedule-count").html(hasSchedule);
            $(".sched-overlay-text").hide();
            $("#sched-overlay-" + data).show();
            if (isEmbed) {
              var pageHeight = Math.max($("body").height(), $("html").height());
              var overlayPos =
                prli.offset().top + $("#sched-overlay-inner").height() / 2;
              if (overlayPos + 250 > pageHeight)
                overlayPos = overlayPos - (overlayPos + 250 - pageHeight + 25);
              $("#sched-overlay-inner").css({ top: overlayPos + "px" });
            }
            $("#sched-overlay").fadeIn(200);
            if (refresh) {
              $("#sched-overlay #dorefresh").val("Y");
            }
          } else if (refresh) location.reload();

          if (data == "free" || data == "filling" || data == "full") {
            $("#sched-page-event-addbutton a").html(
              "&#9733; Add To My Schedule"
            );
            if (data == "free") data = "limited";
            if (seatsBadges[data.toUpperCase()]) {
              seats.removeClass();
              seats.addClass("sched-event-seats");
              seats.addClass("sched-event-seats-" + data);
              seats.html(seatsBadges[data.toUpperCase()]);
            } else {
              seats.remove();
            }
            var seatsstring = data;
            if (data == "limited") seatsstring = "seats available";
            else if (data == "filling") seatsstring = "filling up";
            else if (data == "waitlisted") seatsstring = "full";
            pseats.html(seatsstring);
            if (data == "full")
              psnote.html(
                "Adding this to your schedule will put you on the waitlist."
              );
            if (data != "full" && data != "waitlisted") psnote.hide();
            if (tips[id] != undefined) delete tips[id];
          }
        ...
        $("#sched-confirm-schedule-count")
          ...
          .one("animationend", function () {
            $("#sched-confirm-schedule-count").removeClass("pulse");
          });
        $("#sched-confirm-schedule-text")
          .addClass("pulse")
          .one("animationend", function () {
            $("#sched-confirm-schedule-text").removeClass("pulse");
          });
        $("#sched-confirm-schedule")
          .addClass("pulse")
          .one("animationend", function () {
            $("#sched-confirm-schedule").removeClass("pulse");
          });
        $("#sched-confirm-schedule-count").html(hasSchedule);
        if (hasSchedule > 0) {
          $("#sched-confirm-schedule").slideDown();
        ...
        $.get("/event-set?add=" + id, function (data) {
          if ("object" == typeof data && data["status"]) {
            if (undefined != data["message"]) var message = data["message"];
            if (undefined != data["slot"]) var slot = data["slot"];
            if (undefined != data["plusonenotify"])
              var plusone = data["plusonenotify"];
            var data = data["status"];
          }
          if (data == "no-reload") {
            location.reload();
          ...
          if (
            ...
            if (data != "waitlisted") {
              prli.removeClass("sub").removeClass("plusone");
              $("#sched-page-event-addbutton a").html(
                "&#9733; Add To My Schedule"
              );
              hasSchedule--;
          ...
          } else if (plusone != undefined && plusone == true) {
            $(".sched-overlay-text").hide();
            $("#sched-overlay-plus-one").show();
            if (isEmbed) {
              var pageHeight = Math.max($("body").height(), $("html").height());
              var overlayPos =
                prli.offset().top + $("#sched-overlay-inner").height() / 2;
          ...
          if (
            ...
            pseats.html(seatsstring);
            if (data == "waitlisted") psnote.html("You're on the waitlist.");
            if (data == "full") psnote.show();
          }
          if (typeof slot != "undefined") {
            $.each(slot, function (i, v) {
              $("#" + v)
...
function setupDatePicker(es, ed) {
  ...
  $("#toolbar li.lev1").mouseout(function () {
    if (!$(this).hasClass("ldate")) $("#caldate").dpClose();
  });
}
...
function showToolTip(lnk) {
  ...
  if (el.hasClass("sched-page-grid-event")) {
    ...
    var ty =
      strng.offset().top +
      el.find("strong").height(); /* + parseInt(el.css('margin-top'))*/
  ...
  if (tips[id] == undefined) {
    ...
    tooltipwait = setTimeout(function () {
      $("#tip")
        .html(
          "<div class='popover'><div class='popover-content'><div class='arrow'><span></span></div><div class='popover-body'><div class='popover-body-inner'><div class='sched-meta-loading'><img id='tip-loading' height='24' src='//cdn.sched.co/common/loading-3x.gif' /></div></div></div></div></div>"
        )
        .show()
        .css({ left: tx + "px", top: ty + "px" });
      if (wo) $("#tip .arrow").css("left", wo + "px");
      if (el.hasClass("sched-page-grid-event")) issubbed = "";
      else issubbed = "&issubbed=" + issubbed;
      $.get(
        $("base").attr("href") +
          "event-get?page=" +
          page +
...
function toggleInfo(lnk, id) {
  if ($(lnk).html().substr(0, 4) == L10n["more"]) {
    ...
    if ($("#infobox_" + id).html() == "") {
      ...
      $("#inforow_" + id).show();
      $.get(
        "/event-get?more=" + id + (isEmbed ? "&embed=true" : ""),
        function (ret) {
          $("#infobox_" + id).html(ret);
        }
      );
    } else {
      $("#inforow_" + id).show();
 ...
 * used for contact form popup
 */

var tb_pathToImage = "//cdn.sched.co/common/img/loading.gif";
...
$(document).ready(function () {
  $("#sched-sidebar-filters-custom .li-filter a").click(function () {
    var self = $(this);
...
function validateusername(val) {
  ...
  if (_valun(val)) {
    ...
    unnotify(
      "un-no",
      "Username should contain only latin characters, numbers and periods (no spaces)."
    );
  }
}
...
function isUnameAvailable(uname) {
  ...
  $.get("/profile", { u: uname }, function (d) {
    if ("ok" == d) {
      unok = true;
      unnotify("un-ok", "Username is available");
    } else {
      unok = false;
      unnotify("un-no", "Username not available");
    }
  });
}
...
function toggleExpired(caller) {
  $(".sched-container-hidden").slideToggle(function () {
    if ($(".sched-container-hidden").is(":visible")) {
      $(".sched-container-expired a").html("Hide Earlier Events");
      $(".sched-container-expired a").addClass("hide-past-events");
    } else {
      $(".sched-container-expired a").html("Show Earlier Events");
      $(".sched-container-expired a").removeClass("hide-past-events");
    }
  });
}

// Sponsors Featured column
document.querySelectorAll(".sponsors__section--xxl").forEach((section) => {
  const grid = section.querySelector(".sponsors__grid");
  const items = grid.querySelectorAll(".sponsors__item");

  if (items.length === 1) {
    grid.classList.add("sponsors__grid--one-item");
  } else if (items.length === 2) {
    grid.classList.add("sponsors__grid--two-items");
  }
});
document.querySelectorAll(".sponsors__section--xl").forEach((section) => {
  const grid = section.querySelector(".sponsors__grid");
  const items = grid.querySelectorAll(".sponsors__item");
  ...
  if (items.length === 1) {
    ...
    grid.classList.add("sponsors__grid--three-items");
  }
});
...
(function (a) {
        ...
        detectColours: function (b) {
          var c,
            ...
            v =
              ...
              (g.y === "top" ||
                (g.y === "center" &&
                  f.position().top + n.height / 2 + j.offset <
                    k.titlebar.outerHeight(1))),
            w = v ? k.titlebar : k.tooltip;
          l.addClass(q),
            (o.fill = d = f.css(s)),
            (o.border = e = f[0].style[p] || f.css(m) || l.css(m));
          if (!d || r.test(d))
            (o.fill = w.css(s) || t),
              r.test(o.fill) && (o.fill = l.css(s) || d);
          if (!e || r.test(e) || e === a(document.body).css("color")) {
            o.border = w.css(m) || t;
            if (r.test(o.border) || o.border === w.css("color"))
              o.border = l.css(m) || l.css(p) || e;
          }
          a("*", f)
            .add(f)
            .css("cssText", s + ":" + t + u + ";border:0" + u + ";"),
            l.removeClass(q);
        },
        create: function () {
          var b = n.width,
          ...
          e || (e = m.corner || i.corner),
            B === c
              ...
              : ((B = new g.Corner(B)),
                ...
                B.x === "inherit"
                  ...
                  : B.y === "inherit"
                  ? (B.y = e.y)
                  : B.x === B.y && (B[e.precedance] = e[e.precedance])),
            (D = B.precedance),
            e.precedance === "x" ? t() : u(),
            k.tip.css({ width: (r = n.width), height: (v = n.height) }),
            i.detectColours(e),
            ...
            e.precedance === "y"
              ? (G = [
                  ...
                  C(B.y === "top" ? H.height - v : 0),
                ])
              : (G = [
                  C(B.x === "left" ? H.width - r : 0),
                  C(
                    B.y === "top"
                      ? p
                      : B.y === "bottom"
            ...
            s
              ? (q.attr(H),
                ...
                E.restore(),
                E.save(),
                E.clearRect(0, 0, 3e3, 3e3),
                (E.fillStyle = o.fill),
                (E.strokeStyle = o.border),
                (E.lineWidth = p * 2),
                (E.lineJoin = "miter"),
                (E.miterLimit = 100),
                E.translate(G[0], G[1]),
                E.beginPath(),
                E.moveTo(F[0][0], F[0][1]),
                E.lineTo(F[1][0], F[1][1]),
                E.lineTo(F[2][0], F[2][1]),
                E.closePath(),
                p &&
                  (l.css("background-clip") === "border-box" &&
                    ((E.strokeStyle = o.fill), E.stroke()),
                  (E.strokeStyle = o.border),
                  E.stroke()),
                E.fill())
              : ((F =
            ...
            a.each(l, function (a, c) {
              ...
              c === "center"
                ? ((d = g === "y" ? "left" : "top"),
                  (e[d] = "50%"),
                  (e["margin-" + d] =
                    -Math.round(h[g === "y" ? "width" : "height"] / 2) + f))
                : ((d = w(b, c)),
                  (i = x(b)),
                  (e[c] = a ? 0 : f + (i > d ? i : -d)));
            }),
            (e[b[g]] -= h[g === "x" ? "width" : "height"]),
            d
  ...
  function y(e, h) {
    ...
    l.suppress &&
      (m = a.attr(this, "title")) &&
      a(this).removeAttr("title").attr(t, m).attr("title", ""),
      (i = new x(n, l, e, !!k)),
      a.data(this, "qtip", i),
  ...
  (f = a.fn.qtip =
    ...
    (g.tip = function (a) {
      var b = a.plugins.tip;
      return "object" === typeof b ? b : (a.plugins.tip = new A(a));
    }),
...
$(document).ready(function () {
  ...
  //Resize embed on responsive pages
  $(window).on("resize", function () {
    if (isEmbed) {
      parent.socket.postMessage(document.body.clientHeight);
    }
  });
});
...
function changeTipGrav() {
  ...
  if (tgnew != false) {
    tg = tgnew;
    $(".sched-title-tooltip").qtip({
      style: { classes: "ui-tooltip-bootstrap" },
      position: { my: tg[0], at: tg[1], target: false },
    });
  }
}

$(function () {
  $("#schedorg-header-profile li").hover(
    function () {
      $(this).addClass("hover");
...
function showSurveyForm(evid) {
  if ($(".survey[data-survey-id='" + evid + "']").prop("open") == "true") {
    $(".survey[data-survey-id='" + evid + "']").slideUp(200);
    $(".survey[data-survey-id='" + evid + "']").prop("open", "false");
  } else {
    $(".survey[data-survey-id='" + evid + "']").slideDown(200);
    $(".survey[data-survey-id='" + evid + "']").prop("open", "true");
  }
}

function showAllSurveyForms() {
  $(".survey").show();
  $(".survey").prop("open", "true");
}

function surveySubmit(evid) {
  var validate_ok = true;

  $(survey_requiredinput[evid])
    .each(function (key, val) {
      ...
      switch (type) {
        case "rating":
          ...
          break;
        case "textinput":
          if ($("input[name=" + name + "][data-required]").text().length == 0) {
            //alert(name + " is required");
            $(".sched-overlay-text").hide();
            $("#sched-overlay-survey-missing")
              .html(name + " is required<br>")
    ...
    .promise()
    .done(function () {
      if (!validate_ok) return;

      $.ajax({
        type: "POST",
        url: "/event/" + evid + "?mod=survey",
        data: $(".survey form[data-survey-id='" + evid + "']").serialize(),
        success: function (data) {
          console.debug(data);
          $(".survey-submit[data-survey-id='" + evid + "']")
            .css("display", "block")
            .hide();
          if ("OK" === data.result) {
            console.debug("saved survey response for id: " + evid);
            $(".survey-submit[data-survey-id='" + evid + "']").hide(
              0,
              function () {
                //$(".survey-success[data-survey-id='" + evid + "']").css("display", "block").hide(0, function() {
                $(".survey-submit[data-survey-id='" + evid + "']")
                  .css("display", "block")
                  .show();
                $(".survey-submit[data-survey-id='" + evid + "'] input").val(
                  "Update Feedback"
                );
                $(".survey[data-survey-id='" + evid + "']").slideUp(200);
                $(".survey[data-survey-id='" + evid + "']").prop(
                  "open",
                  "false"
                );
                $(".session-survey-button[data-survey-id='" + evid + "']")
                  .html("Feedback Submitted")
                  .addClass("session-survey-button-reviewed");
                //});
              }
            );
          } else {
            console.debug("error saving survey response for id: " + evid);
            $(".survey-submit[data-survey-id='" + evid + "']")
              .css("display", "block")
              .hide();
            $(".survey-submit[data-survey-id='" + evid + "']").hide(
              0,
              function () {
                $(".survey-failed[data-survey-id='" + evid + "']")
                  .css("display", "block")
                  .delay(2000)
                  .hide(0, function () {
                    $(".survey-submit[data-survey-id='" + evid + "']").show();
                  });
              }
            );
          }
...
$(document).ready(function () {
  $(".survey-sprite").on("click", function (target) {
    $(target.currentTarget).parent().find("input").prop("checked", true);
  });

  // set the mobile menu height properly
  setMobileMenuHeight();
});

// update mobile menu height on orientation change
$(window).bind("resize", function () {
  setMobileMenuHeight();
});

function setMobileMenuHeight() {
  var menuHeight = $("#schedorg-mobile-menu ul").height();
  var screenHeight = $(window).height();
  var menuButtonHeight = $("#schedorg-mobile-link").height();

  if (screenHeight < menuHeight + menuButtonHeight) {
    var menuHeight = screenHeight - menuButtonHeight;
  } else {
    var menuHeight = "auto";
  }

  $("#schedorg-mobile-menu ul").height(menuHeight);
}
...
if (window.location.pathname == "/profile") {
  window.onbeforeunload = function (e) {
    if (!doingSubmit) {
      ...
      if (typeof froalaChanged !== "undefined") {
        hasChanges = true;
      } else {
        $("#js-settings-form input[type='text']").each(function (e, k) {
          if (
            $(k).data("origvalue") !== undefined &&
            $(k).data("origvalue") != $(k).val()
          ) {
            hasChanges = true;
            return;
          }
        });
        $("#js-settings-form input[type='checkbox']").each(function (e, k) {
          if ($(k).data("origvalue") != $(k).is(":checked")) {
            hasChanges = true;
            return;
          }
        });
...
// Sendbird Chat - make Chat icon toggleable
$(".widget.ic-connected.sb-hidden").click(function () {
  if ($(this).hasClass("sb-hidden")) {
    $(".widget.ic-connected").removeClass("sb-hidden");
    $(".widget.ic-connected").addClass("sb-shown");
    $(".channel-board").hide();
    $(".channel-board").removeClass("sb-shown");
    $(".channel-board").addClass("sb-hidden");
  }
});
...
window.addEventListener("load", function () {
  // Show more button
  /* header intro message show more/less toggle */
  ...
  if (typeof textContainer !== "undefined" && textContainer != null) {
    const showMoreButton = document.getElementById(
      "sched-header-message-show-more-button"
    );

    const gradientOverlay = document.getElementById(
      "sched-header-message-gradient-overlay"
    );

    const textContent = textContainer.querySelector(
      ".sched-header-text-container"
    );

    // Check if content exceeds the container height
    function checkContentHeight() {
      // Get the natural height of the content without restrictions
      const contentHeight = textContent.scrollHeight;
      // Get the max height from CSS (120px)
      const maxHeight = parseInt(
        window.getComputedStyle(textContainer).maxHeight
      );

      // Only show the button and gradient if content exceeds max height
      if (contentHeight > maxHeight) {
        showMoreButton.style.display = "flex"; // Show the button
        gradientOverlay.style.display = "block"; // Show the gradient
      } else {
        showMoreButton.style.display = "none"; // Hide the button
        gradientOverlay.style.display = "none"; // Hide the gradient
        textContainer.style.maxHeight = "none"; // Remove height restriction
      }
    }
    ...
    // Also run on window resize in case text reflows
    ...
    if (textContainer && showMoreButton) {
      showMoreButton.addEventListener("click", function () {
        ...
        // Toggle the expanded class on the text container
        ...
        if (textContainer.classList.contains("expanded")) {
          ...
          // If we're collapsing, scroll back to the top
          if (isCurrentlyExpanded) {
            // Using scrollIntoView with smooth behavior
            document.getElementById("sched-body").scrollIntoView({
              behavior: "smooth",
              block: "start",
            });
          }
...
document.addEventListener("DOMContentLoaded", function () {
  // Dropdown
  const viewButtons = document.querySelectorAll(".dropdown-button");

  viewButtons.forEach((viewButton) => {
    const viewDropdown = viewButton.nextElementSibling;

    // Ensure the next sibling is the dropdown menu
    if (viewDropdown && viewDropdown.classList.contains("dropdown-menu")) {
      viewButton.addEventListener("click", function (event) {
        event.preventDefault();

        // Close all dropdowns except the current one
        document.querySelectorAll(".dropdown-menu").forEach((dropdown) => {
          if (dropdown !== viewDropdown) dropdown.style.display = "none";
        });

        // Toggle visibility of the current dropdown
        viewDropdown.style.display =
          viewDropdown.style.display === "block" ? "none" : "block";
      });
    }
  });
...
//// View Dropdown for sessions (Updates select button and Event Schedule links)
window.addEventListener("load", function () {
  const viewButtons = document.querySelectorAll(".session__view-button");
  const viewDropdowns = document.querySelectorAll(".session__view-dropdown");
  const scheduleLinks = document.querySelectorAll(".schedule-dynamic-link");

  // Check cookies for saved layout and redirect if necessary, only if .session__view exists
  const savedLayout = getCookie("selectedLayout");
  /* const sessionViewElement = document.querySelector('.session__view');
  if (savedLayout && sessionViewElement) {
    const targetUrl = getTargetUrl(savedLayout);
    if (!window.location.pathname.includes(targetUrl)){
      window.location.href = targetUrl;
      return; // Exit the function to prevent further code execution on redirect
    }
  } */

  // Update button text and schedule links based on saved layout
  if (savedLayout) {
    /* viewButtons.forEach(button => {
      const span = button.querySelector('span');
      if (span) {
        span.textContent = savedLayout;
      }
    }); */
    updateScheduleLinks(savedLayout);
  }
  const VALID_VIEWS = ["expanded", "grid", "by venue", "map", "simple"];

  viewButtons.forEach((viewButton, index) => {
    const viewDropdown = viewDropdowns[index]; // Assume buttons and dropdowns correspond by index
    const menuLinks = viewDropdown ? viewDropdown.querySelectorAll("a") : null;

    if (viewButton && menuLinks) {
      viewButton.addEventListener("click", function (event) {
        event.preventDefault();
        viewDropdown.style.display =
          viewDropdown.style.display === "block" ? "none" : "block";
      ...
      menuLinks.forEach(function (link) {
        link.addEventListener("click", function (event) {
          ...
          if (VALID_VIEWS.includes(selectedText.toLowerCase())) {
            setCookie("selectedLayout", selectedText, 365); // Set cookie with 365-day expiration
            updateScheduleLinks(selectedText);
          }
          window.location.href = this.href; // Navigate to the selected layout
        });
      });
    }
  });
  ...
  document.addEventListener("click", function (event) {
    viewButtons.forEach((viewButton, index) => {
      const viewDropdown = viewDropdowns[index];
      if (
        viewButton &&
        !viewButton.contains(event.target) &&
  ...
  // Function to get the URL based on the layout view
  function getTargetUrl(view) {
  ...
  // Function to update all schedule links based on the selected layout
  function updateScheduleLinks(view) {
    const newHref = getTargetUrl(view);
    scheduleLinks.forEach(function (link) {
      originalParams = link.href.split("?")[1];
      if (typeof originalParams != "undefined") {
        link.href = newHref + "?" + originalParams;
      } else {
        link.href = newHref;
      }
    });
  ...
  // Utility function to set a cookie with expiration in days
...
// Sticky schedule nav
document.addEventListener("DOMContentLoaded", function () {
  const header = document.querySelector(".schedule__header-container");
  if (!header) return;

  let headerOffset = header.getBoundingClientRect().top + window.pageYOffset;
  const tolerance = 5; // To prevent iOS flicker near the top

  function checkSticky() {
    if (window.pageYOffset >= headerOffset - tolerance) {
  ...
  // Scroll event with passive flag
  let scrollTimeout;
  window.addEventListener(
    ...
    function () {
      ...
      // Recheck after scroll ends (helps with iOS bounce)
      clearTimeout(scrollTimeout);
      scrollTimeout = setTimeout(checkSticky, 100);
    },
    { passive: true }
  );
  ...
  window.addEventListener("resize", function () {
    headerOffset = header.getBoundingClientRect().top + window.pageYOffset;
    checkSticky();
  });
...
// Session Tooltip
function tooltipSessionAdded() {
  // Select all elements with the class 'event-loggedin' containing 'ev-save'
  const saveElements = document.querySelectorAll(
    ".event-loggedin:not(.frozen) .ev-save:not(.disabled)"
  );

  saveElements.forEach((element) => {
    // Find the parent '.event-loggedin' element
    const parent = element.closest(".event-loggedin");

    // Check if a tooltip already exists for the element
    let tooltip = parent.querySelector(".session-tooltip");
    if (!tooltip) {
      // Create a new tooltip element
      tooltip = document.createElement("div");
      tooltip.className = "session-tooltip";
      tooltip.innerText = parent.classList.contains("sub")
        ? "Added to My Schedule"
        : "Removed from My Schedule";

      // Find the '.name' element where the tooltip should be inserted after
      const nameElement = parent.querySelector(".name, .grid-name");
      if (nameElement) {
        nameElement.insertAdjacentElement("afterend", tooltip);
      }
    }

    // Store timeout ID in the element dataset
    element.addEventListener("click", () => {
      // Update the tooltip text based on 'sub' class presence
      tooltip.innerText = parent.classList.contains("sub")
        ? "Added to My Schedule"
        : "Removed from My Schedule";
      ...
      if (element.dataset.tooltipTimeout) {
        clearTimeout(element.dataset.tooltipTimeout);
      }

      // Set a new timeout
      const timeoutId = setTimeout(() => {
        tooltip.classList.remove("visible");
        element.dataset.tooltipTimeout = null;
      }, 2500);
...
// Example of calling the function inside another function
...
document.addEventListener("readystatechange", function () {
  const toggle = document.querySelector(".qr-menu__icon-toggle");
  const modal = document.querySelector(".qr-menu__modal");
  const overlay = document.querySelector(".qr-menu__overlay");
  const closeButton = document.querySelector("#qr-menu-close");
  const scheduleHeader = document.querySelector(".schedule__header-container");
  ...
  function openModal() {
    ...
    // Hide the schedule header if it exists and is fixed
    if (scheduleHeader && scheduleHeader.classList.contains("fixed")) {
  ...
  function closeModal() {
    ...
    // Show the schedule header if it exists
    if (scheduleHeader) {
      if (scheduleHeader.classList.contains("fixed")) {
        scheduleHeader.style.display = "";
      } else {
        scheduleHeader.style.display = "flex"; // Ensure it reappears
      }
    }
  }
...
document.querySelectorAll(".tab-pill").forEach((tab) => {
  tab.addEventListener("click", function (event) {
    ...
    if (activeTab) {
      activeTab.classList.remove("tab-pill--active");
      const activeContent = document.querySelector(activeTab.dataset.tabTarget);
      if (activeContent) {
        activeContent.classList.remove("tab-pill__content--active");
      }
    }
    ...
    if (newContent) {
      newContent.classList.add("tab-pill__content--active");
    }
...
function updateLinksWithParams() {
  ...
  document.querySelectorAll("a.with-params").forEach((link) => {
    const linkParams = new URLSearchParams(link.getAttribute("href"));
    // Merge parameters from the link, replacing existing ones if needed
    linkParams.forEach((value, key) => {
      existingParams.set(key, value);
    });
    link.href = `${currentPath}?${existingParams.toString()}`;
  });
}
...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  if (hash) {
    const tab = document.querySelector('[data-tab-target="'.concat(hash, '"]'));
    if (tab) {
      tab.click();
    }
  }
  // make the links with .with-params class to have the params from the current URL and navigate to current page + params
  updateLinksWithParams();
});
// end Tabs

// Make the "back to schedule" link behave like browser builtin Back button
const backToScheduleLink = document.getElementById("back_to_schedule");
if (backToScheduleLink) {
  backToScheduleLink.addEventListener("click", function (event) {
    event.preventDefault();
    event.stopPropagation();

    const isPreviousPageInternal = document.referrer.includes(
      window.location.host
    );

    const isPreviousPageExempt = document.referrer.includes("/feedback");

    if (
      window.history.length > 1 &&
      isPreviousPageInternal &&
      !isPreviousPageExempt
    ) {
      window.history.back();
    } else {
      window.location.href = backToScheduleLink.href;
    }
  });
}

function toggleTheme() {
  document.body.classList.toggle("schedorg-body-dark");
}
